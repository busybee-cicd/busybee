...
var TestManager = /** @class */ (function() {
  function TestManager(conf, envManager) {
    ...
    if (conf.webSocketPort) {
      ...
      this.wsServer = new TestWebSocketServer_1.TestWebSocketServer(
        wsConf,
        this.envManager
      );
    }
  ...
  TestManager.prototype.buildTestSuiteTasksPromises = function() {
    ...
    conf.parsedTestSuites.forEach(function(testSuite, suiteID) {
      ...
      _this.logger.trace(
        'Processing ' + suiteID + ' : type = ' + testSuite.type
      );
      testSuite.testEnvs.forEach(function(testEnv, suiteEnvID) {
        ...
        if (testSuite.type === SuiteType_1.SuiteType.USER_PROVIDED) {
          envTask = _this.buildEnvTask(
            suiteID,
            testEnv.suiteEnvID,
            SuiteType_1.SuiteType.USER_PROVIDED
          );
          _this.testSuiteTasks[suiteID].envResults.push(envTask);
        } else if (
          testSuite.type === SuiteType_1.SuiteType.REST ||
          _.isUndefined(testSuite.type)
        ) {
          // 1. make sure testSets exist for this testEnv
          if (_.isEmpty(testEnv.testSets)) {
            _this.logger.trace(
              'testEnv ' + testEnv.suiteEnvID + ' contains 0 testSets. skipping'
            );
            return;
          }
          ...
          if (!hasTests_1) {
            _this.logger.trace(
              'testEnv ' + testEnv.suiteEnvID + ' contains 0 tests. skipping'
            );
            return;
          }
          envTask = _this.buildEnvTask(
  ...
  TestManager.prototype.buildEnvTask = function(
    suiteID,
    suiteEnvID,
    suiteType
  ) {
    return __awaiter(this, void 0, void 0, function() {
      ...
      return __generator(this, function(_a) {
        switch (_a.label) {
          case 0:
            this.logger.trace(
              'executeTestEnvTask ' + suiteID + ' ' + suiteEnvID
            );
            generatedEnvID = this.envManager.generateId();
            envResult = EnvResult_1.EnvResult.new(
              suiteType,
              suiteID,
              suiteEnvID
            );
          ...
          case 1:
            ...
            return [
              4 /*yield*/,
              this.envManager.start(generatedEnvID, suiteID, suiteEnvID)
            ];
          case 2:
            _a.sent();
            ...
            if (!(suiteType === SuiteType_1.SuiteType.REST))
              return [3 /*break*/, 4];
            suiteManager = new RESTSuiteManager_1.RESTSuiteManager(
              this.conf,
              currentEnv
            );
          ...
          case 7:
            e_1 = _a.sent();
            this.logger.error(
              'buildEnvTask: ERROR CAUGHT WHILE RUNNING TEST SETS'
            );
          ...
          case 10:
            e2_1 = _a.sent();
            this.logger.error(
              'buildEnvTask: Error Encountered While Stopping ' + generatedEnvID
            );
            return [3 /*break*/, 11];
          case 11:
            return [7 /*endfinally*/];
