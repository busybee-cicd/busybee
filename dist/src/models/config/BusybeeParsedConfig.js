...
var BusybeeParsedConfig = /** @class */ (function() {
  ...
  BusybeeParsedConfig.prototype.parseTestSuites = function(userConf, mode) {
    ...
    if (mode === 'mock') {
      ...
      userConf.testSuites.forEach(function(testSuite) {
        ...
        _this.logger.trace(
          'skipTestSuites: ' + JSON.stringify(_this.skipTestSuites)
        );
        if (
          _.find(_this.skipTestSuites, function(sID) {
            return sID === suiteID;
          })
        ) {
  ...
  BusybeeParsedConfig.prototype.parseTestSuite = function(testSuite, mode) {
    ...
    return new ParsedTestSuiteConfig_1.ParsedTestSuite(
      testSuite,
      mode,
      this.testSet2EnvMap,
  ...
  /*
     ...
     */
  BusybeeParsedConfig.prototype.parseTestFiles = function(
    parsedTestSuites,
    mode
  ) {
    var _this = this;
    ...
    parsedTestSuites.values().map(function(pst) {
      if (pst.testFolder) {
        testFolders.push(
          path.join(_this.filePaths.busybeeDir, pst.testFolder, '/**/*.json')
        );
        testFolders.push(
          path.join(_this.filePaths.busybeeDir, pst.testFolder, '/**/*.js')
        );
      }
    });
    var files = glob.sync('{' + testFolders.join(',') + '}', {
    ...
    files.forEach(function(file) {
      ...
      if (
        _this.testFiles.length > 0 &&
        !_.find(_this.testFiles, function(fileName) {
        ...
        test.testSet.forEach(function(testSetInfo) {
          ...
          testEnvIds.forEach(function(testEnvId) {
            ...
            if (_.isUndefined(testSetInfo.index)) {
              ...
              var existingTests_1 = parsedTestSuites
                .get(suiteID)
                .testEnvs.get(testEnvId)
