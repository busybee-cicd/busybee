  ...
  */
  ...
  async stop(generatedEnvID) {
    return new Promise((resolve, reject) => {
      ...
      execFile(path.join(feenyDir, envInfo.stopScript), [JSON.stringify(args)])
        .then((stdout) => {
  ...
  provisionEnv(generatedEnvID, suiteID, suiteEnvID) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const { stdout, stderr } = await execFile(path.join(feenyDir, testSuiteConf.env.startScript), [JSON.stringify(args)]);
  ...
  confirmHealthcheck(generatedEnvID) {
    return new Promise((resolve, reject) => {
      ...
      if (healthcheckConf.type.toUpperCase() === "REST") {
        ...
        let healthcheckPort;
        if (healthcheckConf.port) {
          healthcheckPort = healthcheckConf.port;
        } else {
