...
class RESTManager {
  constructor(conf, testSuiteConf) {
    this.conf = conf;
    ...
    this.logger.debug(`RESTManager constructor`);
    this.logger.debug(JSON.stringify(testSuiteConf, null, '\t'));
  }

  buildBaseUrl(requestConf, port) {
    this.logger.debug('buildBaseUrl');
    let protocol = this.testSuiteConf.protocol;
    let host = this.testSuiteConf.host;
