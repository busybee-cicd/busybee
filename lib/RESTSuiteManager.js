...
class RESTSuiteManager {
  ...
  async runRESTApiTestSet(currentEnv, testSet, restManager) {
    this.logger.debug(`runRESTApiTestSet ${currentEnv.port} ${testSet.id} ${restManager}`);
  ...
  buildTestTasks(testSet, currentEnv) {
    this.logger.debug(`RESTSuiteManager:buildTestTasks <testSet> ${currentEnv.port}`);
    return testSet.tests.map((test, i) => {
      ...
      return (cb) => {
        ...
        // build request
        let opts = this.restClient.buildRequest(test.request, currentEnv.port);
